...
void CNSSolver::BC_Transpiration_Wall(CGeometry* geometry, CSolver** solver_container, CNumerics* conv_numerics,
                                      CNumerics* visc_numerics, CConfig* config, unsigned short val_marker) {
  BC_Transpiration_Wall_Generic(geometry, config, val_marker);
};

/*======================================================================
Generic transpiration wall BC
======================================================================*/
void CNSSolver::BC_Transpiration_Wall_Generic(const CGeometry* geometry, const CConfig* config,
                                              unsigned short val_marker) {
  constexpr su2double TWO_THIRD  = 2.0 / 3.0;
  constexpr su2double RAMP_ITERS = 999.0;  // use su2double for min()

  const bool implicit     = (config->GetKind_TimeIntScheme() == EULER_IMPLICIT);
  const bool dynamic_grid = config->GetGrid_Movement();

  /*--- optional local Jacobian block (only delete velocity rows, so not really used) ---*/
  su2double** Jac_i = nullptr;
  if (implicit) {
    Jac_i = new su2double*[nVar];
    for (unsigned i = 0; i < nVar; ++i) Jac_i[i] = new su2double[nVar]();  // () → zero-initialise
  }
  ...
  for (unsigned long iVertex = 0; iVertex < geometry->nVertex[val_marker]; ++iVertex) {
    ...
    /*–— 1. normal blowing / suction velocity ε_n –––––*/
    su2double eps_n = nodes->GetTranspiration(iPoint);  // make sure CEulerVariable has this
    if (!config->GetRestart() && !config->GetRestart_Flow()) {
      const su2double iter = static_cast<su2double>(ExtIter);  // ← see note (A) below
      eps_n *= std::min<su2double>(1.0, iter / RAMP_ITERS);
    }

    /*–— 2. geometry – area & inward unit normal –––––*/
    const su2double* Normal = geometry->vertex[val_marker][iVertex]->GetNormal();
    const su2double Area = GeometryToolbox::Norm(nDim, Normal);

    su2double n_hat[MAXNDIM] = {0.0};
    for (unsigned d = 0; d < nDim; ++d) n_hat[d] = -Normal[d] / Area;  // inward-pointing

    /*–— 3. wall velocity (v_grid − ε_n n̂) –––––*/
    su2double v_wall[MAXNDIM] = {0.0};
    if (dynamic_grid) {
      const auto* v_grid = geometry->nodes->GetGridVel(iPoint);
      for (unsigned d = 0; d < nDim; ++d) v_wall[d] = v_grid[d] - eps_n * n_hat[d];
    } else {
      for (unsigned d = 0; d < nDim; ++d) v_wall[d] = -eps_n * n_hat[d];
    }

    /*–— 4. strong Dirichlet for velocity –––––*/
    nodes->SetVelocity_Old(iPoint, v_wall);
    for (unsigned d = 0; d < nDim; ++d) LinSysRes(iPoint, d + 1) = 0.0;
    nodes->SetVel_ResTruncError_Zero(iPoint);

    /*–— 5. primitive vars & gradients –––––*/
    const su2double rho = nodes->GetDensity(iPoint);
    const su2double p = nodes->GetPressure(iPoint);
    const su2double E = nodes->GetEnergy(iPoint);
    const su2double mu = nodes->GetLaminarViscosity(iPoint) + nodes->GetEddyViscosity(iPoint);

    su2double dUdX[3][3] = {{0.0}};
    for (unsigned i = 0; i < nDim; ++i)
      for (unsigned j = 0; j < nDim; ++j) dUdX[i][j] = nodes->GetGradient_Primitive(iPoint, i + 1, j);

    /* divergence and viscous stress */
    su2double div_u = 0.0;
    for (unsigned d = 0; d < nDim; ++d) div_u += dUdX[d][d];

    su2double tau[3][3] = {{0.0}};
    for (unsigned i = 0; i < nDim; ++i)
      for (unsigned j = 0; j < nDim; ++j) {
        const su2double delta_ij = (i == j) ? 1.0 : 0.0;
        tau[i][j] = mu * (dUdX[j][i] + dUdX[i][j]) - TWO_THIRD * mu * div_u * delta_ij;
      }

    su2double tau_dot_v[3] = {0.0};
    for (unsigned i = 0; i < nDim; ++i)
      for (unsigned j = 0; j < nDim; ++j) tau_dot_v[i] += tau[i][j] * v_wall[j];

    /*–— 6. residual updates –––––*/
    su2double Vn = 0.0;  // manual dot product
    for (unsigned d = 0; d < nDim; ++d) Vn += v_wall[d] * n_hat[d];

    const su2double ResRho = rho * Vn * Area;
    const su2double ResE_c = (rho * E + p) * Vn * Area;

    su2double ResE_v = 0.0;
    for (unsigned d = 0; d < nDim; ++d) ResE_v += tau_dot_v[d] * n_hat[d] * Area;

    LinSysRes(iPoint, 0) += ResRho;
    LinSysRes(iPoint, nDim + 1) += ResE_c - ResE_v;

    /*–— 7. Jacobian cleanup –––––*/
    if (implicit) {
      for (unsigned var = 1; var <= nDim; ++var) Jacobian.DeleteValsRowi(iPoint * nVar + var);
    }
  }
  END_SU2_OMP_FOR
  /*------------------------------------------------------------------*/
  ...
  if (Jac_i) {
    for (unsigned i = 0; i < nVar; ++i) delete[] Jac_i[i];
    delete[] Jac_i;
  }
}
...
// void CNSSolver::BC_Transpiration_Wall(CGeometry *geometry, CSolver**,
//                                       CNumerics*, CNumerics*, CConfig *config,
//                                       unsigned short val_marker){
//   BC_Transpiration_Wall_Generic(geometry, config, val_marker);
// }

// void CNSSolver::BC_Transpiration_Wall_Generic(const CGeometry *geometry,
//                                               const CConfig *config,
//                                               unsigned short val_marker)
// {
//   constexpr su2double TWO_THIRD = 2.0/3.0;
//   constexpr unsigned  MFLUX_RAMP = 999;
...
//       Jac_i[i] = new su2double [nVar]();
//   }

//   SU2_OMP_FOR_DYN(OMP_MIN_SIZE)
//   for (unsigned long iVertex = 0; iVertex < geometry->nVertex[val_marker]; ++iVertex){
//     const auto iPoint = geometry->vertexp[val_marker][iVertex]->GetNode();
//     if (!geometry->nodes->GetDomain(iPoint))
//       continue;

//     su2double eps_n = nodes->GetTranspiration(iPoint);
//     if (!config->GetRestart() && !config->GetRestart_Flow()){
//       const auto iter = static_cast<su2double>(config->GetExtIter());
//       eps_n *= std::min(1.0, iter / static_cast<su2double>(MFLUX_RAMP));
//     }
...
//       n_hat[d] = -Normal[d]/Area;

//     su2double v_wall[MAXNDIM] = {0.0};
//     if (dynamic_grid){
//       const auto *v_grid = geometry->nodes->GetGridVel(iPoint);
//       for (unsigned d = 0; d < nDim; d++)
//         v_wall[d] = v_grid[d] - eps_n * n_hat[d];
//     } else {
//       for (unsigned d = 0; d < nDim; d++)
//         v_wall[d] = -eps_n * n_hat[d];
//     }

//     nodes->SetVelocity_Old(iPoint, v_wall);

//     for (unsigned d = 0; d < nDim; d++)
//       LinSysRes(iPoint, d + 1) = 0.0;
//     nodes->SetVel_ResTruncError_Zero(iPoint);
...
//     for (unsigned i = 0; i < nDim; i++)
//       for (unsigned j = 0; j < nDim; j++)
//         dUdX[i][j] = nodes->GetGradient_Primitive(iPoint, i + 1, j);
...
//       for (unsigned j = 0; j < nDim; j++){
//         const su2double delta_ij = (i==j) ? 1.0 : 0.0;
//         tau[i][j] = mu*(dUdX[i][j] + dUdX[i][j]) - TWO_THIRD * mu * div_u * delta_ij;
//       }

//     su2double tau_dot_v[3] = {0.0};
//     for (unsigned i = 0; i < nDim; i++)
//       for (unsigned j = 0; j < nDim; j++)
//         tau_dot_v[i] += tau[i][j] * v_wall[j];

//     const su2double Vn     = GeometryToolbox::Dot(nDim, v_wall, n_hat);
//     const su2double ResRho = rho * Vn * Area;
//     const su2double ResE_c = (rho * E + p) * Vn * Area;

//     su2double ResE_v = 0.0;
//     for (unsigned d = 0; d < nDim; d++)
//       ResE_v += tau_dot_v[d] * n_hat[d] * Area;

//     LinSysRes(iPoint, 0)        += ResRho;
//     LinSysRes(iPoint, nDim + 1) += ResE_c - ResE_v;

//     if (implicit){
//       for (unsigned var = 1; var < nDim; var++)
//         Jacobian.DeleteValsRowi(iPoint * nVar + var);
//     }
//   }
//   END_SU2_OMP_FOR
      ...
      while (fabs(diff) > tol) {
        ...
        if (config->GetMarker_All_KindBC(iMarker) != ISOTHERMAL) {
          ...
          if (denum > EPS) {
            ...
            SU2_OMP_CRITICAL { cout << "Warning: T_Wall < 0 " << endl; }
